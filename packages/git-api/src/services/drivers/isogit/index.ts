  ...
  async createCommit(options: ICommitCreateRequest): Promise<ICommit> {
    const commitResult = await commit({
      ...
      author: options.author ? {
        ...
        timestamp: options.author.date ? Math.floor(new Date(options.author.date).getTime() / 1000) : Math.floor(Date.now() / 1000)
      } : undefined,
      committer: options.committer ? {
        name: options.committer.name,
        email: options.committer.email,
        timestamp: options.committer.date ? Math.floor(new Date(options.committer.date).getTime() / 1000) : Math.floor(Date.now() / 1000)
      } : undefined,
      tree: options.tree,
  ...
  async merge(branch: TBranch, options?: IMergeRequest): Promise<IMergeResult | IMergeStatus> {
    try {
      ...
      return {
        ...
        merged: (mergeResult.alreadyMerged || false) || (mergeResult.fastForward || false) || (mergeResult.mergeCommit || false),
