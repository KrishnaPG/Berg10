...
async function runDebug() {
  ...
  try {
    ...
    try {
      const refs = await repo.listRefs("branch");
    ...
    console.log("\n--- Test 8: Create tag ---");
    try {
      await repo.createTag("v1.0.0" as any, asSha("HEAD"));
    ...
    console.log("\n--- Test 9: List tags ---");
    try {
      const refs = await repo.listRefs("tag");
    ...
    console.log("\n--- Test 10: Get commit log ---");
    try {
      const logResult = await repo.getCommitLog({
        per_page: 10,
      });
    ...
    console.log("\n--- Test 11: Reset ---");
    try {
      await repo.reset(asSha("HEAD"), "hard");
    ...
    console.log("\n--- Test 12: Revert ---");
    try {
      const revertResult = await repo.revert(asSha("HEAD"));
    ...
    console.log("\n--- Test 13: Stage patch ---");
    try {
      await repo.stagePatch(asPath("test.txt"), "This is a patch");
    ...
    console.log("\n--- Test 14: Merge ---");
    try {
      const mergeResult = await repo.merge(asBranch("feature-branch"));
    ...
    console.log("\n--- Test 15: Rebase ---");
    try {
      const rebaseResult = await repo.rebase(asBranch("feature-branch"));
    ...
    console.log("\n--- Test 16: Abort rebase ---");
    try {
      await repo.abortRebase(asBranch("feature-branch"));
