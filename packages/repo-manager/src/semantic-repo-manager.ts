...
import { hashNEncode } from "../../shared/utils";
import {
  ManifestEntry,
  type TBlobSha256,
  type TEntityId,
...
export class SemanticRepoManager {
  ...
  private blobPath(sha: TBlobSha256, ext: string = ".blob") {
    return path.resolve(this.root, "index", "blobs", ...shaToFilePath(sha, ext));
  }
  ...
  async getGroup(name: string) {
    try {
      const buf = await readFile(join(this.groupDir(name), "config.json"), "utf8");
  ...
  async updateGroup(name: string, patch: Partial<TSemanticGroupConfig>) {
    ...
    await atomicWrite(join(this.groupDir(name), "config.json"), JSON.stringify(next, null, 2));
    return next;
  }
  ...
  async readManifestFile(laneSha: TLaneSha, entityId: TEntityId): Promise<TManifestEntry | null> {
  ...
  async writeCache(groupSha: string, commitSha: string, entities: any[]) {
    const path = join(this.root, "cache", groupSha, `${commitSha}.entities.jsonl.zst`);
